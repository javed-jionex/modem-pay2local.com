...
export class DepositsComponent {
  ...
  paymentsList() {
    ...
    this.depositsService.list(data).subscribe((res: any) => {
  ...
  downloadDocument(type: string) {
    let data = {
      ...
      bank_number_id: this.searchParm?.bank_number_id || null,
      ...
      duration: this.searchParm?.duration || null,
      end_date: this.searchParm?.end_date || null,
    };
    this.depositsService.downloadFile(data).subscribe((res: any) => {
      if (res.status == 200) {
